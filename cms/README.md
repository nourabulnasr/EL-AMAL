...
Hosted setup added 19 September: a separate el-amal-hosted database and owner now exist. Production enablement was explicitly requested by Nour. Hosted credentials are in ignored .env.hosted-owner.local, separate from development. Consult PROGRESS.md for deployment verification status. The bootstrap accepts CMS_BOOTSTRAP=hosted as an explicit alternative, still refuses any existing staff records, and never resets users.
