...
// No transport is configured by default. This module never sends directly or schedules itself.
...
export async function deliverNextNotification(payload:Payload,transport?:NotificationTransport){
 ...
 SELECT id FROM notifications WHERE source='cms' AND attempts<5 AND
 ...
 try{
   ...
   return {outcome:result.rowCount?'retry' as const:'stale' as const};
 }
}
