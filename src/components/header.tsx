...
export function Header({locale}:{locale:Locale}){
 ...
 <nav id="main-navigation" className={open?'navigation navigation-open':'navigation'} aria-label={locale==='en'?'Main navigation':'التنقل الرئيسي'}><Link aria-current={path.includes('/products')||path.includes('/categories')?'page':undefined} href={`/${locale}/products`} onClick={()=>setOpen(false)}>{t.catalogue}</Link><Link href={`/${locale}#industries`} onClick={()=>setOpen(false)}>{t.industries}</Link><Link href={`/${locale}/rfq`} onClick={()=>setOpen(false)}>{locale==='ar'?'طلب عرض سعر':'Request a quote'}</Link><Link href={`/${locale}#approach`} onClick={()=>setOpen(false)}>{t.about}</Link></nav>
