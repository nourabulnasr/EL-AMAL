...
export function TestEnquirySubmit({details,locale,active}:{details:EnquiryDetails;locale:Locale;active:boolean}){
  ...
  const signature=JSON.stringify({locale,contact:details,lines});
  ...
  return <div className="review-notice">
    ...
    <button type="button" className="button button-dark" disabled={busy} onClick={async()=>{
      ...
      try {
        const response=await fetch('/api/enquiry-submissions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requestKey,locale,contact:details,lines})});
