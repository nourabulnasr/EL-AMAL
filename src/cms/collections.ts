...
import {hasRole,canPublish} from '../lib/access';
...
const bilingual=(name:string,type:'text'|'textarea'='text'):Field=>({name,type:'group',fields:[{name:'en',label:'English',type,required:true},{name:'ar',label:'Arabic',type,required:true}]});
export const Staff:CollectionConfig={
 ...
 fields:[{name:'role',type:'select',required:true,defaultValue:'catalogue-editor',options:['owner','catalogue-editor','sales','warehouse'],access:{update:({req})=>hasRole(req.user,['owner'])}}],
};
...
export const Products:CollectionConfig={
 ...
 fields:[{name:'externalId',type:'text',required:true,unique:true},{name:'model',type:'text',required:true,index:true},{name:'category',type:'relationship',relationTo:'categories',required:true},bilingual('name'),bilingual('description','textarea'),{name:'sourceRef',type:'text',required:true,access:{read:({req})=>!!req.user}},{name:'reviewedBy',type:'relationship',relationTo:'staff',access:{read:({req})=>hasRole(req.user,['owner','catalogue-editor']),update:({req})=>hasRole(req.user,['owner'])}},{name:'reviewedAt',type:'date',access:{update:({req})=>hasRole(req.user,['owner'])}},{name:'rightsConfirmed',type:'checkbox',defaultValue:false,access:{update:({req})=>hasRole(req.user,['owner'])}}],
};
