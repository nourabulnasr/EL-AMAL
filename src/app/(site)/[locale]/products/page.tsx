...
export default async function Page({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{q?:string;category?:string}>}){
 const {locale}=await params;if(!isLocale(locale))notFound();const s=await searchParams;
 return <CatalogueView locale={locale} query={typeof s.q==='string'?s.q.slice(0,120):''} category={typeof s.category==='string'?s.category:''}/>;
}
